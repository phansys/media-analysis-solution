...
class StatusModal extends Component {
  ...
  getStatus() {
    ...
    this.interval = setInterval(function () { getStateMachineStatus() }, 5000);
  ...
  render() {
    const console_link = [media_analysis_config.SOLUTION_CONSOLE_LINK, this.props.objectid].join(':');
    const result_link = ["/result", this.props.objectid].join('/');

    if (null === this.interval || null === this.props.format) {
      return null;
    }
